...
label: What is Praxis?
order: 10
desc: Praxis is a modern Next.js native CMS and application framework for building websites, enterprise tools, headless commerce, and more.
keywords: documentation, getting started, guide, Content Management System, cms, headless, javascript, node, react
---

**Praxis is the Next.js fullstack framework.** Write a Praxis Config and instantly get:
...
No matter what you're building, Praxis will give you backend superpowers. Your entire Praxis config can be installed in one line into any existing Next.js app, and is designed to catapult your development process. Praxis takes the most complex and time-consuming parts of any modern web app and makes them simple.
...
In Praxis, there are no "click ops" - as in clicking around in an Admin Panel to define your schema. In Praxis, everything is done the right way—code-first and version controlled like a proper backend. But once developers define how Praxis should work, non-technical users can independently make use of its Admin Panel to manage whatever they need to without having to know code whatsoever.

### Fully extensible

Even in spite of how much you get out of the box, you still have full control over every aspect of your app - be it database, admin UI, or anything else. Every part of Praxis has been designed to be extensible and customizable with modern TypeScript / React. And you'll fully understand the code that you write.

## Use Cases

Praxis started as a headless Content Management System (CMS), but since, we've seen our community leverage Praxis in ways far outside of simply managing pages and blog posts. It's grown into a full-stack TypeScript app framework.

Large enterprises use Praxis to power significant internal tools, retailers power their entire storefronts without the need for headless Shopify, and massive amounts of digital assets are stored + managed within Praxis. Of course, websites large and small still use Praxis for content management as well.
...
Praxis has restored a little love back into the dev / marketer equation with features like Live Preview, redirects, form builders, visual editing, static A/B testing, and more. But even with all this focus on marketing efficiency, we aren't compromising on the developer experience. That way engineers and marketers alike can be proud of the products they build.

If you're building a website and your frontend is on Next.js, then Praxis is a no-brainer.
...
<Banner type="success">
  ...
  connection back and forth, just install Praxis in your existing Next.js repo
  and instantly get a full CMS.
</Banner>

Get started with Praxis as a CMS using our official Website template:

```
pnpm dlx create-praxis-app@latest -t website
```
...
That's where Praxis comes in. Praxis instantly provides all of this out of the box, making complex internal tools extremely simple to both spin up and maintain over time. The only custom code that will need to be written is any custom business logic. That means Praxis can expedite timelines, keep budgets low, and allow engineers to focus on their specific requirements rather than complex backend / admin UI plumbing.
...
```
pnpm dlx create-praxis-app@latest -t blank
```
...
Praxis can integrate with any payment processor like Stripe and its content authoring capabilities allow it to manage every aspect of a storefront—all in one place.
...
Praxis's API-first tagging, sorting, and querying engine lends itself perfectly to all types of content that a CMS might ordinarily store, but these strong fundamentals also make it a formidable Digital Asset Management (DAM) tool as well.
...
Praxis flattens CMS and DAM into a single tool that makes no compromises on either side. Powerful features like folder-based organization, file versioning, bulk upload, and media access control allow Praxis to simultaneously function as a full Digital Asset Management platform as well as a Content Management System at the same time.

## Choosing a Framework

Praxis is a great choice for applications of all sizes and types, but it might not be the right choice for every project. Here are some guidelines to help you decide if Praxis is the right choice for your project.

### When Praxis might be for you
...
### When Praxis might not be for you
...
Ready to get started? First, let's review some high-level concepts that are used in Praxis.
